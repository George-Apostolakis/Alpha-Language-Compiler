...
/* These are owned by the lexer but declared here for growStringBuffer */
...
/* Helper: decide which TypeHint suffix to print */
...
void printTokens(TokenList head, FILE *out) {
    ...
    for (Token *cur = head; cur; cur = cur->next) {
        ...
        if (cur->tokenCharacteristic == NOCHARACTERISTIC) {
